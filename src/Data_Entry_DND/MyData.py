...
class MyData:
    ...
    def __init__(self):
        ...
        generated by the on_selection_changed method, which detected which
        selection (from the sorted or unsorted treeview) and converts the
        path references from the sorted view to those needed to manipulate
...
#            self.window.reshow_with_initial_size()
    ...
    def on_delete_button_clicked(self, widget): # pylint: disable-msg = W0613
        ...
        underlying unsorted ListStore. Note that every time there's a 
        change to the ListStore through moving, adding or deleting a record, the
    ...
    def on_open_menu_item_activate(self, widget):
        ...
        if response == Gtk.ResponseType.OK:
            ...
            try:
                ...
                Block disk_rewrite until we're done so we don't rewrite the 
                entire file for every row inserted into the ListStore from the
                disk file.
                '''
