...
# produced the signal.
        row = [item for item in self.CurrentRecordsStore[path]]
...
# column number of the cell that produced the signal.
...
# cast it as an int - no bounds checking needed here. .               
        if isinstance(widget, Gtk.CellRendererSpin):
            row[col_num] = int(text)
        else:           
            row[col_num] = text
# Now submit the local copy of the existing record, now containing the new, updated
# data, to a function that checks the entire record for invalid entries. If the 
# updated record fails the error check, the on_records_edited method returns without
# changing the existing record; if the error check is passed, the updated record gets
# written to the ListStore.
        if not Error_Check(self, row):
            return
        else:
            self.CurrentRecordsStore[path][col_num] = row[col_num]
